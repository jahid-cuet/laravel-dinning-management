    ...
    <div class="row mb-4">
        <div class="col">
            <div class="card text-center p-3">
                <h5>{{ $total_meals }}</h5>
                <p>Today's Total Meals</p>
            </div>
        </div>
        <div class="col">
            <div class="card text-center p-3">
                <h5>{{ $total_lunch }}</h5>
                <p>Today's Total Lunch</p>
            </div>
        </div>
        <div class="col">
            <div class="card text-center p-3">
                <h5>{{ $total_dinner }}</h5>
                <p>Today's Total Dinner</p>
            </div>
        </div>
    </div>
                    ...
                    <table class="table" id="">
                        ...
                        <tbody>
                            ...
                            {{-- @foreach ($data as $row)
                                <tr>
                                    ...
                                    <td><span class="text-dark-75 font-weight-bold d-block font-size-lg">
                                            ...
                                            @if ($row->meal_date != '')
                                                {{ date('d M Y', strtotime($row->meal_date)) }}
                                            @else
                                                n/a
                                            @endif
                            ...
                            @foreach ($data as $meal)
                                ...
                                <td>{{ $meal->dinningStudent->student_id ?? 'N/A' }}</td>
                                <td>{{ $meal->dinningStudent->name ?? 'N/A' }}</td>
                                <td>{{ $meal->dinningStudent->studentSession->name ?? 'N/A' }}</td>
                                <td>{{ $meal->dinningStudent->department->name ?? 'N/A' }}</td>
                                <td>{{ $meal->lunch == 1 ? 'Yes' : 'No' }}</td>
                                <td>{{ $meal->dinner == 1 ? 'Yes' : 'No' }}</td>
                                    </td>
                                </tr>
                                @php
                                    $serial++;
                            ...
                            {{-- <td>
                                        <div class="form-check form-switch form-switch-md">
                                            <input type="checkbox" name="lunch" value="{{ $row->id }}"
                                                onclick="toggleSwitchStatus(this,'meals');" class="form-check-input"
                                                @if ($row->lunch == 1) checked @endif>
                                        </div>
                                    ...
                                    <td>
                                        <div class="form-check form-switch form-switch-md">
                                            <input type="checkbox" name="dinner" value="{{ $row->id }}"
                                                onclick="toggleSwitchStatus(this,'meals');" class="form-check-input"
                                                @if ($row->dinner == 1) checked @endif>
                                        </div>
                                ...
                                <ul class="trk-action__list">
                                    <li class="trk-action">
                                        <a class="trk-action__item trk-action__item--success"
                                            href="{{ route('admin.meals.show', $row->id) }}">
                                    ...
                                    @can('meal-delete')
                                        <li class="trk-action">
                                            <a onclick="Delete(`{{ route('admin.meals.destroy', $row->id) }}`)"
